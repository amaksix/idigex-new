    ...
    <div class="container">
      <div class="sec-head mb-80">
        <div class="d-flex align-items-center justify-end">
          <div>
            <span class="sub-title mb-15 opacity-8">- {{ t('home.services.section.sub_title') }}</span>
            <h3 class="text-u f-bold fz-50">
              {{ t('home.services.section.title') }}
              <span class="f-ultra-light">{{ t('home.services.section.title_light') }}</span>
            </h3>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div class="row">
          <div class="col-lg-6 cont">
            <div class="info">
              <span class="tag">{{ t(`home.services.${item.key}.tag`) }}</span>
              <span class="date">{{ item.subtitle }}</span>
            </div>
            <h3 class="text-u">{{ t(`home.services.${item.key}.title`) }}</h3>
          </div>
          <div class="col-lg-3 offset-lg-3 d-flex align-items-center justify-end">
            <div class="ml-auto">
              <NuxtLink
                :to="localePath(item.readMoreLink)"
                class="more mt-15 animsition-link"
              >
                <span>
                  {{ t('home.services.section.read_more') }}
